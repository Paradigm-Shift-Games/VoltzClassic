...
module.Explode = function(position, radius, damage)
	...
	if radius > 0 and damage > 0 then
		...
		for v, _ in pairs(modelCache) do
			if v.Name == "Shield" or v.ClassName == "Folder" or v.ClassName == "SpawnLocation" then continue end
			local distance = getDistance(v, position)
			local blastDamage = damage * getDamageMultiplier(distance, radius)
			if not v.PrimaryPart then v.PrimaryPart = v:FindFirstChildOfClass("Part") end
			if not v.PrimaryPart then error(v, "has no primarypart!") end
