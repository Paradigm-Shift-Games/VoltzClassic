...
local TransferLine = require(game.ServerScriptService.Packages.Electricity.Classes.NetworkObjects.TransferLine)
...
local ArrayGraph = require(game.ServerScriptService.Classes.Datastructure.ArrayGraph)
...
--Stats
local StructureStats = require(game.ReplicatedStorage.Stats.Structure)
local FabricatorStats = StructureStats["Fabricator"]
local BlueprintStats = StructureStats["Fabricator Blueprint"]
...
--Connections
CollectionService:GetInstanceAddedSignal("Fabricator"):Connect(OnFabricatorAdded)
CollectionService:GetInstanceAddedSignal("Blueprint"):Connect(OnBlueprintAdded)
CollectionService:GetInstanceRemovedSignal("Fabricator"):Connect(OnRemoved)
...
function runObject(Fabricator)
	local team = Fabricator.Team.Value
	local hardConnected = hardGraph:GetConnections(Fabricator)
	local softConnected = softGraph:GetConnections(Fabricator)
	...
	for i = softConnected:GetSize(), 1 , -1 do
		...
		Disconnect(softGraph, Fabricator, Blueprint)
	end
...
module.Run = function(delta)
	...
	--Run All Fabricators
	for _, v in pairs(CollectionService:GetTagged("Fabricator")) do runObject(v) end
end
